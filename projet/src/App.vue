<template>
  <div id="app">
    <h4 class="bg-danger text-white text-center p-2">Hello {{ name }} - Please Set up this projet</h4>
    <div class="container-fuild p-4">
      <div class="row" v-if="filteredTasks.length == 0">
        <div class="col text-center">
          <b>Nothing to do {{ name }}</b>
        </div>
      </div>
      <template v-else>
        <div class="row">
          <div class="col-6 font-weight-bold">Task</div>
          <div class="col-6 font-weight-bold">Done</div>
        </div>
        <div class="row" v-for="t in filteredTasks" v-bind:key="t.action">
          <div class="col-6">{{t.action}}</div>
          <div class="col-6 text-center">
            <input type="checkbox" v-model="t.status" class="from-check-input" />
          </div>
        </div>
      </template>
      <div class="row py-2">
